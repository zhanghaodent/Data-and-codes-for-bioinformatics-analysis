...
rm(list=ls())
inputFile="GSE126805_Cibersort.txt"       #输入
outFile="GSE126805_barplot.pdf"       #输出
setwd("")    #工作目录

#读取文件，整理
data=read.table(inputFile,sep="\t",header=T,row.names=1,check.names=F)
rownames(data) <- factor(rownames(data),levels = rownames(data),ordered = T)
data <- data[,-1]
