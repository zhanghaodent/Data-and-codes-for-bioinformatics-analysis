...
do.tissueDist <- function(cellInfo.tb = cellInfo.tb,
                          meta.cluster = cellInfo.tb$meta.cluster,#####更改细胞名称
...
setwd("D:\\ischemic_reperfusion\\GSE161201\\8.OR值")
mouse_data <- readRDS("mergeTEN_2_after_anno.rds")
mouse_data@meta.data[1:5,]
meta.data <- mouse_data@meta.data
colnames(meta.data)
library(tidyverse)
fix(meta.data)#######需要把细胞类型的列名改为meta.cluster，组别列名改为loc
out.prefix <- "./Fig1"
OR.CD8.list <- do.tissueDist(cellInfo.tb=meta.data,
