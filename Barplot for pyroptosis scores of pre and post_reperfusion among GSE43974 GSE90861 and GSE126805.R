...
library(reshape2)
library(ggpubr)
inputFile="GSE1291666_keygene.txt"      #输入文件
outFile="GSE129166_IFNG_gene.pdf"      #输出文件
setwd("D:\\抗体介导排斥课题\\生信分析\\3.正式绘图\\13.多个基因_箱线图")     #修改工作目录
...
data <- read.table("IFNG_gene.txt",sep="\t",header=T,check.names=F,row.names=1)
gene <- intersect(rownames(data),colnames(rt))
...
library(reshape2)
library(ggsci)
setwd("D:\\缺血再灌注课题\\正式绘图_ER_stress\\7.三个基因的表达量图")  
